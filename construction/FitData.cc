...
namespace RHD
{
     ...
     * test statistic which is based on the likelihood ratio with respect to
     * the saturated model.
     ...
     */
    {
        ...
        if (use_toys) {
            std::cout << "---- Using toys to perform BC goodness-of-fit test. ----" << std::endl;
            ...
            TF1* fit = pdf.asTF(RooArgList(*obs), RooArgList(*params), *obs);
            //TF1* fit = new TF1("t", "gaus", 40, 200);

            // Distribution of test statistic generated by toy
            ...
            while (trial++ <= ntrials) {
                ...
                double minfcn = r->MinFcnValue();
                std::cout << minfcn << std::endl;
                toy_BC_values.push_back(2. * r->MinFcnValue());
                TCanvas c;
                htoy->Draw();
                fit->Draw("SAME");//
                c.SaveAs(Form("toy_N%d.jpg", trial));
                delete htoy;
                delete binned_toy;
            }
            ...
            if (_SAVEOPTION) {
                TH1F toy_distribution("toy_dist", "toy_dist", 100, 0, 10000);
                                      //50 * ((int)(toy_BC_values.front()-50)%50),
                                      //50 * ((int)(toy_BC_values.back()+100)%50));
    ...
    double FitData::getBCTestResult ( RooRealVar& ObsVar,
                                      Roo)
    {
        ...
        double pdf_total_integral = pdf.createIntegral(ObsVar, RooFit::NormSet(ObsVar))->getVal();
        ...
        for (int bin=1; bin <= nbins; bin++) {
            ...
            if (bin_content > 0) {
                ...
                ObsVar.setRange(range_name,
                                ...
                                bin_center+bin_width/2.);
                double pdf_val = pdf.createIntegral(ObsVar,
                                                    RooFit::NormSet(ObsVar),
                                                    RooFit::Range(range_name))->getVal();
                double expected = N_expected*pdf_val/pdf_total_integral;
                BC_test += 2*(expected - bin_content
                              + bin_content*TMath::Log(bin_content/expected));
            }
        }
        
        // Get asymptotic probability
        std::cout << "chi2: " << BC_test << std::endl;
        std::cout << "dof: " << nbins_nonzero - params->getSize() << std::endl;
        double asym_prob = TMath::Prob(BC_test, nbins_nonzero - params->getSize());

        // TEST: alternative definition of GoF test stat (chi square)
        RooPlot* plot_chi2 = ObsVar.frame();
        data.plotOn(plot_chi2, Binning(ObsVar.getBinning()), Name("data"));
        pdf.plotOn(plot_chi2, Name("pdf"));

        std::cout << "Real chi2: " << plot_chi2->chiSquare(params->getSize()) << std::endl;
    }
