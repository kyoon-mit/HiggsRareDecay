...
namespace RHD
{
        ...
        if (use_toys) {
            ...
            while (trial++ <= ntrials) {
                ...
                std::vector<double> toy_stats
                    = getBCTestResult<RooDataHist>(ObsVar, pdf, *binned_toy);
                toy_BC_values.push_back(toy_stats[0]);
            }
    ...
    template<typename T>
    std::vector<double> FitData::getBCTestResult ( RooRealVar& ObsVar,
                                                    RooAbsPdf& pdf,
                                                            T& data )
    /* Get values of Baker-Cousins test statistic. Returns std::vector of size 3.
     * Elements are: BC test statistic, degrees of freedom, and chi2/dof.
     */
    {
        ...
        TH1* hist = data.createHistogram(data.GetName(), ObsVar,
        ...
        for (int bin=1; bin <= nbins; bin++) {
            ...
            if (bin_content > 0) {
                ...
                double pdf_integral
                    = pdf.createIntegral(ObsVar,
                                         RooFit::NormSet(ObsVar),
                                         RooFit::Range(range_name))->getVal();
                
                double expected = N_expected*pdf_integral/pdf_total_integral;
        ...
        // std::cout << "Real chi2: " << plot_chi2->chiSquare(dim_params) << std::endl;

        std::vector<double> results{BC_test,
                                    static_cast<double>(dof),
                                    plot_chi2->chiSquare(dim_params)};
