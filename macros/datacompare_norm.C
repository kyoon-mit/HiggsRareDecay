...
void datacompare_norm(const char* Meson) {
    ...
    ggH_sig_stack.GetYaxis()->SetTitle("Normalized Events/0.1 GeV");
    c_ggH_hist.BuildLegend(0.9, 0.9, 0.55, 0.83);
    c_ggH_hist.SaveAs(Form("compare/presel_normalized_signal_ggH_hist_compare_GF_%s_JUL24.png", Meson));
    c_ggH_hist.Close();
    ...
    VBF_sig_stack.GetYaxis()->SetTitle("Normalized Events/0.1 GeV");
    c_VBF_hist.BuildLegend(0.9, 0.9, 0.55, 0.83);
    c_VBF_hist.SaveAs(Form("compare/presel_normalized_signal_VBF_hist_compare_GF_%s_JUL24.png", Meson));
    c_VBF_hist.Close();

    // TCanvas c_comb_hist;
    // comb_sig_stack.Draw("nostack p0");
    // comb_sig_stack.GetXaxis()->SetTitle(Form("m_{#%s#gamma} [GeV/c^{2}]", meson));
    // comb_sig_stack.GetYaxis()->SetTitle("Normalized Events/0.1 GeV");
    // c_comb_hist.BuildLegend(0.95, 0.9, 0.55, 0.83);
    // c_comb_hist.SaveAs(Form("compare/presel_normalized_signal_comb_hist_compare_GF_%s_JUL24.png", Meson));
    // c_comb_hist.Close();
    ...
    c_data_hist.SaveAs(Form("compare/presel_normalized_bkg_observed_data_hist_compare_GF_%s_JUL24.png", Meson));
    c_data_hist.Close();
}
