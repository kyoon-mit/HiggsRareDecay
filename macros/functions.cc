...
bool checkRhoTracks(const Vec_i& genPart_pdgId, Vec_i& genPart_genPartIdxMother) {

  int idxHiggs = -1;
  // loop over all the genPartCand
  for (unsigned int i=0; i<genPart_pdgId.size(); i++) {
    if (genPart_pdgId[i]==25) idxHiggs = i;
  }

  int idxRho = -1;
  for (unsigned int i=0; i<genPart_pdgId.size(); i++) {
    if (genPart_pdgId[i]==113 and genPart_genPartIdxMother[i]==idxHiggs) idxRho=i;
  }

  int idxTrack1 = -1;
  int idxTrack2 = -1;
  for (unsigned int i=0; i<genPart_pdgId.size(); i++) {
    if (genPart_pdgId[i]==211 and genPart_genPartIdxMother[i]==idxRho) idxTrack1=i;
    else if (genPart_pdgId[i]==-211 and genPart_genPartIdxMother[i]==idxRho) idxTrack2=i;
  }

  bool foundMother = (idxTrack1!=-1 && idxTrack2!=-1);
  return foundMother;
}

bool checkPhiTracks(const Vec_i& genPart_pdgId, Vec_i& genPart_genPartIdxMother) {
  ...
  int idxPhi = -1;
  for (unsigned int i=0; i<genPart_pdgId.size(); i++) {
    if (genPart_pdgId[i]==333 and genPart_genPartIdxMother[i]==idxHiggs) idxPhi=i;
  }

  int idxTrack1 = -1;
  int idxTrack2 = -1;
  for (unsigned int i=0; i<genPart_pdgId.size(); i++) {
    if (genPart_pdgId[i]==321 and genPart_genPartIdxMother[i]==idxPhi) idxTrack1=i;
    else if (genPart_pdgId[i]==-321 and genPart_genPartIdxMother[i]==idxPhi) idxTrack2=i;
  }
...
float getPhiPolarizationAngle(const Vec_i& genPart_pdgId, Vec_i& genPart_genPartIdxMother,
  ...
  bool foundMother = (idxHiggs!=-1 && idxPhi!=-1 && idxTrack1!=-1 && idxTrack2!=-1);
  ...
  if (foundMother) {
    ...
    p_kplus.SetPtEtaPhiM(genPart_pt[idxTrack1], genPart_eta[idxTrack1],
                         genPart_phi[idxTrack1], genPart_mass[idxTrack1]);
