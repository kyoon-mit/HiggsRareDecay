...
void datacompare_mva(const char* Meson) {
    ...
    // Files
    const char* Torino_presel_filename = Form("/work/submit/mariadlf/cards_march9/%s_%sGammaLimit/workspace_STAT_%s_GFcat_bdt0_2018.root", date.c_str(), Meson, Meson);
    const char* MIT_presel_sig_filename = Form("/work/submit/mariadlf/cards_march9/WSmva_MARCH9/Signal_GFcat__%sCat_2018_workspace.root", Meson);
    const char* MIT_presel_bkg_filename = Form("/work/submit/mariadlf/cards_march9/WSmva_MARCH9/Bkg_GFcat__%sCat_2018_workspace.root", Meson);
    ...
    std::cout << "Torino (March 9): " << Torino_ggH_signal_norm << ", MIT (March 9): " << MIT_ggH_signal_norm << std::endl;
    std::cout << "GFcat " << Meson << "Cat signal VBF normalizations (MVA) " << std::endl;
    std::cout << "Torino (March 9): " << Torino_VBF_signal_norm << ", MIT (March 9): " << MIT_VBF_signal_norm << std::endl;
    std::cout << "GFcat " << Meson << "Cat background normalizations (MVA) " << std::endl;
    std::cout << "Torino (March 9): " << Torino_data_bkg_norm << ", MIT (March 9): " << MIT_data_bkg_norm << std::endl;
    std::cout << "*************************************************" << std::endl;
    ...
    // PDFs
    // RooAbsPdf* Torino_ggH_pdf = Torino_workspace->pdf(Form("crystal_ball_%s_GFcat_bdt0_ggH", Meson));
    // RooAbsPdf* Torino_VBF_pdf = Torino_workspace->pdf(Form("crystal_ball_%s_GFcat_bdt0_VBF", Meson));
    // RooAbsPdf* Torino_bkg_pdf1 = Torino_workspace->pdf("chebychev_GFcat_bdt0_bkg");
    ...
    TH1F* MIT_ggH_signal = new TH1F("MIT_ggH_signal", "MIT (March 9)", 100., 170., 70);

    TH1F* Torino_VBF_signal = new TH1F("Torino_VBF_signal", "Torino", 100., 170., 70);
    TH1F* MIT_VBF_signal = new TH1F("MIT_VBF_signal", "MIT (March 9)", 100., 170., 70);

    TH1F* Torino_comb_signal = new TH1F("Torino_comb_signal", "Torino", 100., 170., 70);
    TH1F* MIT_comb_signal = new TH1F("MIT_comb_signal", "MIT (March 9)", 100., 170., 70);

    TH1F* Torino_data_bkg = new TH1F("Torino_data_bkg", "Torino", 100., 170., 70);
    TH1F* MIT_data_bkg = new TH1F("MIT_data_bkg", "MIT (March 9)", 100., 170., 70);
    ...
    // c_ggH_fit.SaveAs(Form("compare/mva_signal_ggH_fit_compare_GF_%s.png", Meson));
    // c_ggH_fit.Close();

    // TCanvas c_VBF_fit("c", "c", 1200, 800);
    // VBF_frame->Draw();
    // c_VBF_fit.BuildLegend(0.9, 0.9, 0.65, 0.75);
    // c_VBF_fit.SaveAs(Form("compare/mva_signal_VBF_fit_compare_GF_%s.png", Meson));
    // c_VBF_fit.Close();
    
    // TCanvas c_data_fit("c", "c", 1200, 800);
    // bkg_frame->Draw();
    // c_data_fit.BuildLegend(0.37, 0.27, 0.12, 0.12);
    // c_data_fit.SaveAs(Form("compare/mva_bkg_observed_data_fit_compare_GF_%s.png", Meson));
    // c_data_fit.Close();
    ...
    c_data_hist.BuildLegend(0.95, 0.9, 0.7, 0.75);
