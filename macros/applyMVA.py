...
def applyMVA (prodMode, decayChannel, data=False):
    ...
    outFileFormat = '/work/submit/kyoon/RareHiggs/test/MVAoutput/2023/{traindate}/{prod}_{decay}_mvas/disc/2018/outname_mc{num}_{cat}_{Decay}Cat_{year}.root'
    ...
    if prodMode=='GF' and decayChannel=='phi':
        ...
        if data==False:
            nums, years = list(range(9, 20)) + [-65, -66, 1010], [2018]
            nVars = 13
    elif prodMode=='VBFlow' and decayChannel=='rho':
        specification['cat'] = 'VBFcatlow'
        if data==False:
            nums, years = list(range(9, 20)) + [-65, -66, 1020], [2018]
            nVars = 13
